...
<title>Il tuo abbonamento sta per scadere — Kalendarium</title>
</head>
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.07);">
        ...
        <tr>
          <td style="padding:40px 40px 32px;">
            <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#1a1a2e;">Il tuo abbonamento sta per scadere</h1>
            <p style="margin:0 0 16px;font-size:15px;color:#555;line-height:1.6;">
              L'abbonamento di <strong>{{ $organization->name }}</strong> scade il
              <strong>{{ $subscription->paid_period_ends_at?->format('d/m/Y') }}</strong>
              (tra {{ $subscription->daysLeftInPaidPeriod() }} giorn{{ $subscription->daysLeftInPaidPeriod() === 1 ? 'o' : 'i' }}).
            </p>
            <table width="100%" cellpadding="12" cellspacing="0" style="background:#fff8f0;border:1px solid #D4724A;border-radius:8px;margin-bottom:24px;">
              <tr>
                <td style="font-size:14px;color:#a04020;">
                  <strong>Come rinnovare</strong><br>
                  Effettua un bonifico bancario prima della scadenza e scrivici con il riferimento del pagamento per un rinnovo senza interruzioni.
                </td>
              </tr>
            </table>
