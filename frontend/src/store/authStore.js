...
export const useAuthStore = create((set) => ({
  ...
  loginWithToken: async (token) => {
    ...
    try {
      ...
      const userResponse = await auth.me();
      set({ token, user: userResponse.data, isAuthenticated: true, isLoading: false });
      return { success: true };
    } catch (error) {
      localStorage.removeItem('token');
      set({ isLoading: false });
      return { success: false, error: 'Errore validazione token' };
    }
  },
  ...
  checkAuth: async () => {
    ...
    if (!token) return set({ isAuthenticated: false, authInitialized: true });
    try {
      const response = await auth.me();
      set({ user: response.data, isAuthenticated: true, authInitialized: true });
    } catch {
      localStorage.removeItem('token');
